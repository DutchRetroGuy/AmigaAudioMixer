; $VER: CallbackExample.asm 1.0 (05.02.24)
;
; CallbackExample.asm
...
.main_lp
		...
		; Handle callback settings & callback triggering
		bsr		HandleCallback
...
.cb_up
		; Change Callback value up
		move.w	cb_position,d0
		beq		.done
		
		subq.w	#4,d0
		move.w	d0,cb_position
		bra		.done
...
; Callback routines
;------------------------------------
		; Routine: CallBackExample
		; This routine is called when a sample ends on any of the channels.
		; It writes a '1' to the callback trigger, which is handled in the
		; main loop.
		;
		; Note: all Callback functions get passed the sample pointer of the
		;       sample that just ended in A0 and the mixer channel that was
		;       used in D0.
		;
		; Note: apart from D0, callback functions have to preserve register
		;       content.
		;
		; Returns
		; D0 - zero if no sample was started on the same channel, non-zero if
		;      a sample was started on the same channel.
CallBackExample
		movem.l	a0/a1/a2,-(sp)					; Stack
		lea.l	cb_trigger,a1
		move.w	#1,(a1)
		
		movem.l	(sp)+,a0/a1/a2				; Stack
		rts

		; Routine: HandleCallback
		; This routine handles the callback event, both from the UI as well as
		; any callback that has been triggered.
HandleCallback	
		movem.l	d7/a0/a1,-(sp)				; Stack
		
		; Handle changes in callback setting
		tst.w	cb_position
		beq.s	.disable_callback
		
		; Set up the callback routine
		lea.l	CallBackExample(pc),a0
		bsr		MixerEnableCallback
		bra.s	.check_callback
		
.disable_callback
		; Disable the callback routine
		bsr		MixerDisableCallback
		
.check_callback
		; Handle triggered callback routine
		tst.w	cb_trigger
		beq.s	.no_callback_trigger
		
		; Reset callback trigger
		clr.w	cb_trigger
		move.w	cb_colour_count,cb_counter
...
effect_struct_cb	blk.b	mfx_SIZEOF

		; Callback variables
cb_trigger		dc.w	0
cb_counter		dc.w	0

		; Callback colour flash values
cb_colour_count	dc.w	12
cb_colours		dc.w	$300,$522,$844,$a88,$daa,$fff,$daa,$a88
				dc.w	$844,$522,$300,$000
